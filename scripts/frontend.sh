...
# Check if a URL/domain is responding (single attempt)
check_app_status() {
	local domain=$1
	if curl -sS --head --fail --max-time 5 "$domain" > /dev/null; then
...
# Retry until a URL responds, or fail after timeout
# Usage: wait_for_url <url> <timeout_seconds> <label>
wait_for_url() {
	local url=$1
	...
	echo "Waiting for $label to respond (timeout: ${timeout_seconds}s)..."
	while [ "$elapsed" -lt "$timeout_seconds" ]; do
		if curl -sS --head --fail --max-time 5 "$url" > /dev/null 2>&1; then
			echo "$label is responding (after ${elapsed}s)"
			return 0
		fi
		sleep "$interval"
...
# Wait for PM2 process + local port before touching the old server
...
# Require the new instance to be healthy on its local port before cutover
if ! wait_for_url "http://localhost:$DEPLOYMENT_PORT" 45 "Port $DEPLOYMENT_PORT"; then
	echo "ERROR: New deployment is not responding on port $DEPLOYMENT_PORT"
	echo "Checking PM2 logs for $DEPLOYING_TO_NAME:"
	pm2 logs $DEPLOYING_TO_NAME --lines 20 --nostream
	echo "Checking if port is in use:"
	netstat -tlnp | grep :$DEPLOYMENT_PORT || echo "Port $DEPLOYMENT_PORT not in use"
	exit 1
fi

# Give Caddy time to mark the new upstream healthy while old is still up
echo "Waiting for Caddy to detect new healthy server..."
sleep 10

# Domain should still be healthy with both (or old + new) available
if ! wait_for_url "$DOMAIN" 30 "Domain (pre-cutover)"; then
	echo "ERROR: Domain is not responding before cutover. Debugging..."
...
echo "Domain is responding with new deployment available"
...
if [ "$CURRENT_LIVE_NAME" != "false" ]; then
	if check_pm2_process $CURRENT_LIVE_NAME; then
		...
		# Caddy health checks can take 10-30s to fail over after upstream stop
		if wait_for_url "$DOMAIN" 45 "Domain (post-cutover)"; then
			echo "Deployment completed successfully - domain is responding after cutover"
		else
			echo "WARNING: Domain still not responding after cutover wait"
			echo "Verifying new instance is still healthy locally..."
			if wait_for_url "http://localhost:$DEPLOYMENT_PORT" 10 "Port $DEPLOYMENT_PORT"; then
				echo "New instance is healthy; Caddy may still be catching up"
				echo "Checking Caddy status:"
				systemctl is-active caddy || echo "Caddy is not active"
				echo "Checking Caddy logs (last 10 lines):"
				journalctl -u caddy -n 10 --no-pager || true
				# One more longer wait before declaring failure
				if ! wait_for_url "$DOMAIN" 30 "Domain (final recovery)"; then
					echo "ERROR: Domain did not recover after stopping old server"
					exit 1
				fi
			else
				echo "ERROR: New instance also stopped responding - rolling back"
				pm2 start $CURRENT_LIVE_NAME || echo "Failed to restart previous version"
				exit 1
			fi
		fi
...
if ! wait_for_url "$DOMAIN" 30 "Domain (final check)"; then
